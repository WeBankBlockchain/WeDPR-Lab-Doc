.. container:: row 
   
   .. container:: card-holder-bigger

         .. container:: card-bigger

            .. image:: _static/images/wedpr_logo.png



========

WeDPR(星隐)是基于区块链、安全多方计算、联合建模等技术构建一个密态隐私计算系统，可实现“原始数据不出域，数据可用不可见、可控可计量”，服务公共数据授权运营、金融联合风控营销、跨行业数据合作等场景。

========

.. toctree::
   :maxdepth: 1
   :caption: 平台介绍

   docs/introduction/introduction.md
   docs/introduction/key_feature.md
   docs/introduction/function_overview.md
   docs/introduction/change_log/index.rst

========

.. toctree::
   :maxdepth: 1
   :caption: 快速开始

   docs/quick_start/hardware_requirements.md
   docs/quick_start/standalone_installation.md

========

.. toctree::
   :maxdepth: 1
   :caption: 用户指南

   docs/manual/interface_ui.md
   docs/manual/dataset_op.md
   docs/manual/project_op.md
   docs/manual/wizard_mode_op.md
   docs/manual/expert_mode_op.md
   docs/manual/service_publish_op.md
   docs/manual/credential_mgr_op.md
   docs/manual/user_mgr.md
   docs/manual/wepdr_mpc_dev.md

========

.. toctree::
   :maxdepth: 1
   :caption: 运维指南

   docs/op/index.rst
   docs/op/pre_installation.md
   docs/op/wedpr_builder.md
   docs/op/compile.md
   docs/op/config/index.rst

========

.. toctree::
   :maxdepth: 1
   :caption: 系统设计

   docs/design/architecture.md

========

.. toctree::
   :maxdepth: 1
   :caption: 应用接入

   docs/sdk/gateway_sdk/index.rst
   docs/sdk/ml_sdk/index.md
   docs/sdk/pir_sdk/index.md

   docs/api/index.rst

.. toctree::
   :hidden:
   :maxdepth: 1
   :caption: 常见问题

   docs/faq/function.md
   docs/faq/perf.md
   docs/faq/deploy.md


.. toctree::
   :hidden:
   :maxdepth: 1
   :caption: 社区

   docs/community/contract_us.md
